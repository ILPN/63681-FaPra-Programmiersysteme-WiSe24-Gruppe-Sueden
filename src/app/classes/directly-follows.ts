export class DirectlyFollows {
    ...
    private rekursivePathDepthSearch(originNode: string,
                                     wanted: Set<string>,
                                     visited: Set<string>,
        ...
        let successorNodeSet = this.getSuccessor(originNode);
        if (successorNodeSet) {
            //gehe alle Folgeknoten durch
            for (let successorNode of successorNodeSet) {
                //Prüfe bereits gesucht
                if (!visited.has(successorNode)) {
                    if (this.rekursivePathDepthSearch(successorNode, wanted, visited, nodesWithPath, allowedNodes)) {
    ...
    existsPath(firstNodeSet: Set<string>, secondNodeSet: Set<string>, allowedNodes?: Set<string>,): boolean {
        ...
        for (let node of firstNodeSet) {
            ...
            if (this.rekursivePathDepthSearch(node, secondNodeSet, visited, nodesWithPath, allowedNodes)) {
